...
public class CardController {
    ...
    // 카드 생성
    @PostMapping
    public ResponseEntity<CardResponse> createCard(@RequestBody CardRequest cardRequest, @AuthenticationPrincipal UserDetails userDetails) {
        CardResponse cardResponse = cardService.createCard(cardRequest, userDetails.getUsername());
        return new ResponseEntity<>(cardResponse, HttpStatus.CREATED);
    }

    // 카드 수정
    @PutMapping("/{cardId}")
    public ResponseEntity<CardResponse> updateCard(@PathVariable Long cardId, @RequestBody CardRequest cardRequest, @AuthenticationPrincipal UserDetails userDetails) {
    ...
    public ResponseEntity<String> updateCardPosition(@PathVariable Long cardId, @RequestParam int newPosition, @RequestParam Long newColumnId,
                                                     @AuthenticationPrincipal UserDetails userDetails) {
